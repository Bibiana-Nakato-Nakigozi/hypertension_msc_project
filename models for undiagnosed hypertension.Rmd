...
numeric_diagnosed <- combined_undiagnosed %>%
  mutate(
...
combined_undiagnosed[categorical_vars] <- lapply(combined_undiagnosed[categorical_vars], as.factor)

# Converting the hypertension_type to a binary factor 1= with undiagnosed else is without undiagnosed hypertension
combined_undiagnosed$hypertension_type <- as.factor(ifelse(combined_undiagnosed$hypertension_type == 1, 1, 0))

# Checking class distribution
print("Class distribution in UndiagnosedHypertension:")
print(table(combined_undiagnosed$hypertension_type))

# Splitting data into training and testing sets
set.seed(123)
train_index <- createDataPartition(combined_undiagnosed$hypertension_type, p = 0.8, list = FALSE)
train_data <- combined_undiagnosed[train_index, ]
test_data <- combined_undiagnosed[-train_index, ]
...
set.seed(123)
cv_folds <- createFolds(combined_undiagnosed$hypertension_type, k = 5, list = TRUE, returnTrain = FALSE)
...
for (i in 1:5) {
  ...
  train_data <- combined_undiagnosed[-cv_folds[[i]], ]
  valid_data <- combined_undiagnosed[cv_folds[[i]], ]
...
# Ploting ROC curve for test set
...
# Calculating and displaying the final performance metrics
test_sensitivity <- test_conf_matrix$byClass["Sensitivity"]
test_specificity <- test_conf_matrix$byClass["Specificity"]
test_ppv <- test_conf_matrix$byClass["Pos Pred Value"]
test_npv <- test_conf_matrix$byClass["Neg Pred Value"]

cat("\n--- Final Model Performance on Test Set ---\n")
cat("Test Sensitivity:", test_sensitivity, "\n")
cat("Test Specificity:", test_specificity, "\n")
cat("Test Positive Predictive Value:", test_ppv, "\n")
cat("Test Negative Predictive Value:", test_npv, "\n")
...
set.seed(123)
cv_folds <- createFolds(numeric_diagnosed$hypertension_type, k = 10, list = TRUE)
